...
\null
\vfill
\textsc{\Large Experimenter Instructions for Subjects}\\[0.5cm]
\HRule \\[0.7cm]
...
\tikzstyle{key} = [draw=black, very thick, rectangle, rounded corners, minimum size=36pt,
font=\Large]
\tikzstyle{space key} = [key, minimum width=36ex]

\node[space key] (Space) {\textbf{Space Bar}};

\node[key] (J) [above right=of Space] {\textbf{J}};
\node[key] (K) [right=of J] {\textbf{K}};
\node[key] (L) [right=of K] {\textbf{L}};

\node[key] (D) [above left=of Space] {\textbf{D}};
\node[key] (W) [above left=of D] {\textbf{W}};
\node[key] (A) [below left=of W] {\textbf{A}};
...
Surrounding the fortress, different symbols will appear (``\textbf{\#}'', ``\textbf{@}'',
``\textbf{\$}'', ``\textbf{\&}''). Please point to the symbol now. When an ``\textbf{\#}'' is followed
by an ``\textbf{\$}'', you have the opportunity to obtain more resources. You can choose to get 100
...
If you press one of these buttons after the ``\textbf{\$}'' that follows an ``\textbf{\#}'', and
before the next symbol appears, you will get the bonus that you selected and you will hear a chime.
However, if you press one of these two buttons at any other time, including after an ``\textbf{\#}''
but before the next symbol appears, you will hear a buzzing sound and lose 50 Bonus points.
Furthermore, if you hit one of the buttons following an ``\textbf{\#}'' and the next symbol happens
to be an ``\textbf{\$}'', hitting one of the bonus keys will have no effect.
