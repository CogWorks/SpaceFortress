...
def get_config():
    ...
    config.add_setting('General', 'human', True, type=config.editor.CT_CHECKBOX, about='Will this game be played by a human or a cognitive model? Set to f to disable human control, enabling ACT-R')
    config.add_setting('General', 'fullscreen', False, type=config.editor.CT_CHECKBOX, about='Run at full screen? Set to f to run in a window')
    config.add_setting('General', 'linewidth', 1, 'Width of lines drawn on screen. Increase for a more "projector-friendly" game')
    config.add_setting('General', 'psf', False, type=config.editor.CT_CHECKBOX, about='Original PSF (SF4) settings? If t, *ALL* further values are ignored')
    ...
    config.add_setting('General', 'print_events', False, type=config.editor.CT_CHECKBOX, about='Print events to stdout')
    config.add_setting('General', 'sound', True, type=config.editor.CT_CHECKBOX, about='Enable/disable sound')
    
    config.add_setting('Keybindings', 'thrust_key', 'w', type=config.editor.CT_COMBO, options=PYGAME_KEYS)
    config.add_setting('Keybindings', 'left_turn_key', 'a', type=config.editor.CT_COMBO, options=PYGAME_KEYS)
    config.add_setting('Keybindings', 'right_turn_key', 'd', type=config.editor.CT_COMBO, options=PYGAME_KEYS)
    config.add_setting('Keybindings', 'fire_key', 'SPACE', type=config.editor.CT_COMBO, options=PYGAME_KEYS)
    config.add_setting('Keybindings', 'IFF_key', 'j', type=config.editor.CT_COMBO, options=PYGAME_KEYS)
    config.add_setting('Keybindings', 'shots_key', 'k', type=config.editor.CT_COMBO, options=PYGAME_KEYS)
    config.add_setting('Keybindings', 'pnts_key', 'l', type=config.editor.CT_COMBO, options=PYGAME_KEYS)
    ...
    config.add_setting('Hexagon', 'hex_shrink', False, type=config.editor.CT_CHECKBOX, about='Does the outer hex shrink over time?')
    config.add_setting('Hexagon', 'hex_shrink_radius', 150, 'If hex shrinks, to what minimum radius?')
    
    config.add_setting('Fortress', 'fortress_exists', True, type=config.editor.CT_CHECKBOX, about='Does the fortress exist?')
    ...
    config.add_setting('Fortress', 'hit_fortress_while_mine', False, type=config.editor.CT_CHECKBOX, about="Can the fortress be hit if there's a mine onscreen?")
    ...
    config.add_setting('Bonus', 'bonus_exists', True, type=config.editor.CT_CHECKBOX, about='Do bonuses exist?')
    config.add_setting('Bonus', 'bonus_system', "AX-CPT", type=config.editor.CT_LINEEDIT, about='Bonus system standard or AX-CPT?')
    config.add_setting('Bonus', 'randomize_bonus_pos', False, type=config.editor.CT_CHECKBOX, about='Randomize bonus position?')
    ...
    config.add_setting('Bonus', 'bonus_symbol', '$', type=config.editor.CT_LINEEDIT, n=1, about='Bonus symbol')
    config.add_setting('Bonus', 'non_bonus_symbols', '!&*%@', type=config.editor.CT_LINEEDIT, about="Non-bonus symbols. Defaults are # & * % @. Don't use '-', because that's used in the log file to represent that there's no symbol present")
    ...
    config.add_setting('Mine', 'mine_exists', True, type=config.editor.CT_CHECKBOX, about='Do mines exists?')
    config.add_setting('Mine', 'mine_mode', 'standard', type=config.editor.CT_LINEEDIT, about='Set mine behavior to standard or MOT')
    ...
    config.add_setting('Mine', 'fortress_resets_mine', True, type=config.editor.CT_CHECKBOX, about='Does destroying the fortress reset the mine timer?')
    ...
    config.add_setting('MOT', 'MOT_identification_type', 'shots', type=config.editor.CT_LINEEDIT, about='MOT identification through shots or ship? (i.e., fly into the mine)')
    ...
    config.add_setting('MOT', 'MOT_movement_style', 'bounce', type=config.editor.CT_LINEEDIT, about='Do MOT mines bounce off walls or warp around edges?')
    ...
    config.add_setting('Score', 'new_scoring', False, type=config.editor.CT_CHECKBOX, about='Use the new scoring system for Flight, Fortress, and Mines? (instead of PNTS, CNTRL, VLCTY and SPEED)')
    config.add_setting('Score', 'new_scoring_pos', True, type=config.editor.CT_CHECKBOX, about='New scoring position is a more "eye-tracker friendly" format that places the scores around the perimeter of the gameworld, rather than just all along the bottom. Set to f for default positioning')
