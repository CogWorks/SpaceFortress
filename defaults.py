...
def get_config():
    ...
    cfg.add_setting('General', 'human', True, type=config.editor.CT_CHECKBOX, about='Will this game be played by a human or a cognitive model? Set to f to disable human control, enabling ACT-R')
    cfg.add_setting('General', 'fullscreen', False, type=config.editor.CT_CHECKBOX, about='Run at full screen? Set to f to run in a window')
    cfg.add_setting('General', 'linewidth', 1, 'Width of lines drawn on screen. Increase for a more "projector-friendly" game')
    cfg.add_setting('General', 'psf', False, type=config.editor.CT_CHECKBOX, about='Original PSF (SF4) settings? If t, *ALL* further values are ignored')
    cfg.add_setting('General', 'game_time', 30000, 'Time in milliseconds for a game. NOTE! If you escape in the middle of a game, the log will have "short" prepended to the name')
    cfg.add_setting('General', 'games_per_session', 8, 'Number of games per "session"')
    cfg.add_setting('General', 'print_events', False, type=config.editor.CT_CHECKBOX, about='Print events to stdout')
    cfg.add_setting('General', 'sound', True, type=config.editor.CT_CHECKBOX, about='Enable/disable sound')
    
    cfg.add_setting('Keybindings', 'thrust_key', 'w', type=config.editor.CT_COMBO, options=PYGAME_KEYS)
    cfg.add_setting('Keybindings', 'left_turn_key', 'a', type=config.editor.CT_COMBO, options=PYGAME_KEYS)
    cfg.add_setting('Keybindings', 'right_turn_key', 'd', type=config.editor.CT_COMBO, options=PYGAME_KEYS)
    cfg.add_setting('Keybindings', 'fire_key', 'SPACE', type=config.editor.CT_COMBO, options=PYGAME_KEYS)
    cfg.add_setting('Keybindings', 'IFF_key', 'j', type=config.editor.CT_COMBO, options=PYGAME_KEYS)
    cfg.add_setting('Keybindings', 'shots_key', 'k', type=config.editor.CT_COMBO, options=PYGAME_KEYS)
    cfg.add_setting('Keybindings', 'pnts_key', 'l', type=config.editor.CT_COMBO, options=PYGAME_KEYS)
